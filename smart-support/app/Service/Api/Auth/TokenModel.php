...
class TokenModel
{
	...
	public function validate(){
		return ctype_digit($this->userId) &&
			   ...
			   $this->tokenTypeId == Smart\TokenType::TYPE_BEARER_ID &&
	...
	public function toJwt(){
		return TokenJwt::buildToken($this);
	}
	...
	public static function fromJwt($jwt){
		$model = new static();
		$model->userId = $jwt->getClaim('aud');
		$model->tokenTypeId = $jwt->getClaim('sub');
		$model->unique = $jwt->getClaim('jti');
		$model->created = Carbon::createFromTimestamp($jwt->getClaim('iat'));
		$model->expired = Carbon::createFromTimestamp($jwt->getClaim('exp'));
		return $model;
	}
