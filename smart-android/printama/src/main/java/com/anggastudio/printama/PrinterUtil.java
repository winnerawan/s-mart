...
class PrinterUtil {
    ...
    private static final byte[] OPEN_CASH_DRAWER1 = new byte[]{0x1B, 0x78, 0x01};
    private static final byte[] OPEN_CASH_DRAWER2 = new byte[]{0x1B, 0x78, 0x02};
    ...
    void cashDrawer() {
        try {
            btOutputStream.write(OPEN_CASH_DRAWER1);
            btOutputStream.write(OPEN_CASH_DRAWER2);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
     ...
     * https://stackoverflow.com/questions/76635506/how-to-send-esc-pos-pulse-command-to-open-cashe-drawer
     */
    public void openCashDrawer() {
        try {
//            cashDrawer();
//            final byte[] openCD={27,112,0,60,120};
//            btOutputStream.write(openCD);
//            byte[] bytes = intArrayToByteArray(new int[]{27, 112, 0, 50, 250});
//            btOutputStream.write(bytes);
//            btOutputStream.write(OPEN_CASH_DRAWER1);
//            btOutputStream.write(OPEN_CASH_DRAWER2);
//            btOutputStream.write(new byte[]{0x07});
//            btOutputStream.write(new byte[]{0x1b,0x45,0x01}); // Bold
            btOutputStream.write(new byte[]{0x1B, 0x64, 0x02}); // Cut
            btOutputStream.write(new byte[]{0x1B, 0x70, 0x00, (byte) 0xFF, (byte) 0xFF}); // Open cash drawer
            btOutputStream.close();
        } catch (IOException e) {
